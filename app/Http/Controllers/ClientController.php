...
class ClientController extends Controller
{
    ...
    public function activeJob(Request $request){
        ...
        $job->isAvailable = 1;
        $job->save();
        return redirect()->back()->with('success', 'Successfully activated post');  
    }

    public function inActiveJob(Request $request){
        $id = $request->id;
        $job = Job::findOrFail($id);
        $job->isAvailable = 0;
        $job->save();
        return redirect()->back()->with('success', 'Successfully inactivated');  
    }
}